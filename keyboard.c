...
static char keyboard_read(void)
{
    ...
    if (flag & E0_SIGN) { /* last scan code is e0 */
        flag &= ~E0_SIGN;
        return 0;
    }

    if (scan_code & 0x80) { /* checking when shift key is still pressed */
        flag &= ~(shift_code[scan_code]);
        return 0;
    }

    flag |= shift_code[scan_code];
    flag ^= lock_code[scan_code]; /* xor since capslock is active when only press once */
...
{
    char ch[2] = { 0 }; /* reason we made 2-length array is print function not support single charcter */
